...
- **Results** — таблица рекордов: самые быстрые и самые долгие серии.  
- **Language switcher (ENG/RU/ET)** — переключатель языка на главном экране.  
...
| ViewModel | Назначение | Коментарий |
|------------|-------------|-------------|
| **MenuViewModel** | Управление главным меню, выбор языка, начало игры | В планах пока пустая |
| **GameViewModel** | Логика игрового процесса, счёт, рецепт, прогресс | В планах пока пустая |
| **ResultViewModel** | Отображение таблицы результатов и обновление данных | Реализована |

---

## 🪄 Особенности

- Полная **локализация** через `AppResources.resx` (английский, русский и эстонский).  
- **Анимации и эффекты в игре (масштабирование котла, тряска при ошибках) с помощью встроенных анимаций .NET MAUI (ScaleTo, TranslateTo и др.).
